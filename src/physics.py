...
class Physics():
	...
	def __init__(self, config):
		...
		self.sim_duration = 0
		self.sim_model_state = {"pos":{"x":0, "y":0, "z":0}, \
			"ang":{"x":0, "y":0, "z":0, "w":0}}
...
class Gazebo(Physics):
	...
	def start_ros(self):
		...
		if self.ros_status == psutil.STATUS_RUNNING \
			or self.ros_status == psutil.STATUS_SLEEPING:
				print("ROS is already started")
				return

		self.ros_ps = subprocess.Popen([self.ros_ps_name], \
			shell=False)
	...
	def kill_ros(self):
		...
		for proc in psutil.process_iter():
			if proc.name() == self.ros_ps_name or \
				proc.name() == "rosmaster" or proc.name() == "rosout":
					proc.kill()
